...
def home():
    ...
    averages = {
        subj: round(sum(scores) / len(scores), 1) if scores else 0
        for subj, scores in subject_averages.items()
    }
...
@app.route('/report/<name>', methods=["GET", "POST"])
def report(name):
    student = next((s for s in students_data if s['name'].lower() == name.lower()), None)
    if not student:
        return f"No report found for {name}", 404

    if request.method == "POST":
        # Allow updating remarks inside the report page
        new_remarks = request.form.get("remarks", "").strip()
        student["remarks"] = new_remarks
        save_records()
    ...
    grades = {
        subj: convert_grade(student['subjects'].get(subj, 0))
        for subj in subjects
    }
...
def login():
    if request.method == "POST":
        ...
        if user.lower() == "teacher" and pw == "password123":
